...
DECLARE 
   ...
   names namesarray; --declaro una variable del tipo que he definido arriba
   marks grades; 
   total integer; 
BEGIN 
   names := namesarray('Kavita', 'Pritam', 'Ayan', 'Rishav', 'Aziz'); --el tipo que definimos es como el constructor
   marks:= grades(98, 97, 78, 87, 92); 
   total := names.count; --aki los arrays no van desde cero hasta el total-1, sino desde 1 hasta el total?
   dbms_output.put_line('Total '|| total || ' Students'); 
   FOR i in 1 .. total LOOP 
      dbms_output.put_line('Student: ' || names(i) || 'Marks: ' || marks(i)); --variable y posicion
   END LOOP; 
END; 
