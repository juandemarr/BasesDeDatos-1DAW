...
create table gamasproductos(
    gama varchar2(50 byte) primary key,
...
create table productos(
    codigoproducto varchar2(15 byte) primary key,
...
create table oficinas(
    ...
    lineadireccion2 varchar2(50 byte)
);

create table empleados(
    codigoempleado number(*,0) primary key,
...
create table clientes(
    codigocliente number(*,0) primary key,
    nombrecliente varchar2(50 byte),
    nombrecontacto varchar2(30 byte),
    apellidocontacto varchar2(30 byte),
    ...
    pais varchar2(50 byte),
    codigopostal varchar2(10 byte),
    codigoempleadorepventas number(*,0),
...
create table pedidos(
    codigopedido number(*,0) primary key,
    fechapedido date,
    fechaesperada date,
    fechaentrega date,
...
--1.- Realizar un bloque anónimo que dado el código de un cliente nos muestre su nombre.

set serveroutpu on;
declare

    codigo clientes.codigocliente%type:=&introduce_código_cliente;
    nombre clientes.nombrecliente%type;

begin

    select nombrecliente into nombre from clientes where codigo=codigocliente;
    dbms_output.put_line(nombre);

    exception
        when no_data_found then
            dbms_output.put_line('No se encuentra registrado el código');
...
declare
    ...
    codigo productos.codigoproducto%type:='&introduce_el_codigo_del_producto';
    cursor cursorr is select precioventa,gama from productos where codigoproducto=codigo;
    precio productos.precioventa%type;
...
begin
    ...
    loop
        fetch cursorr into precio,gama;
            dbms_output.put_line('El precio es: '||precio||'€ y la gama es: '||gama);
        exit when cursorr%ROWCOUNT=1;
    end loop;
...
declare
    ...
    codigo productos.codigoproducto%type:='&introduce_el_codigo_de_producto';
    precio productos.precioventa%type;
    gama productos.gama%type;

begin

    select precioventa,gama into precio,gama from productos where codigoproducto=codigo;
    dbms_output.put_line('El precio es: '||precio||'€ y la gama: '||gama);
...
--en vivero
describe clientes;
--hacer un procedimiento que ponga en pantalla los datos de los clientes
--que viven en una determinada ciudad que se le pasa como argumento
--y devuelva el numero de clientes que viven en la misma ciudad
--usar rowtype
...
set serveroutput on;
declare
    v_ciudad clientes.ciudad%type:='Madrid';--:='&introduce_ciudad'
    v_numero int;
begin
    dbms_output.put_line('Los clientes de la ciudad '||v_ciudad||' son los siguientes:');
